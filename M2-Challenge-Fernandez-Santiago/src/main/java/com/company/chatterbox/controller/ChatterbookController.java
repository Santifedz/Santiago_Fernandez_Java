...
import com.company.chatterbox.ChatterPost;
import com.company.chatterbox.User;
...
public class ChatterbookController {
    ...
    // Rout to get individual user by username
    // http method: get
    // uri: /user/{username}
    @RequestMapping(value="/user/{username}", method=RequestMethod.GET)
    public User getUserByUsername(@PathVariable String username){
        User target = userList.stream().filter(u -> u.getName().equals(username)).findFirst().orElse(null);
        return target;
    }

    // Rout to get list of chatterPosts for a user by username
    // http method: get
    // uri: /user/{username}
    @RequestMapping(value="/userChatterPosts/{username}", method=RequestMethod.GET)
    public List<ChatterPost> getChatterPostsByUsername(@PathVariable String username){
        User target = userList.stream().filter(u -> u.getName().equals(username)).findFirst().orElse(null);

        if(target == null) return null;

        List<ChatterPost> targetPosts = target.getChatterPosts();
        return target.getChatterPosts();
    }
